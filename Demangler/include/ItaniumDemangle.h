...
public:
  Node(Kind K_, Cache RHSComponentCache_ = Cache::No, Cache ArrayCache_ = Cache::No, Cache FunctionCache_ = Cache::No)
      : K(K_), RHSComponentCache(RHSComponentCache_), ArrayCache(ArrayCache_), FunctionCache(FunctionCache_) {}
  ...
  bool hasRHSComponent(OutputStream &S) const {
    if (RHSComponentCache != Cache::Unknown) return RHSComponentCache == Cache::Yes;
    return hasRHSComponentSlow(S);
  }

  bool hasArray(OutputStream &S) const {
    if (ArrayCache != Cache::Unknown) return ArrayCache == Cache::Yes;
    return hasArraySlow(S);
  }

  bool hasFunction(OutputStream &S) const {
    if (FunctionCache != Cache::Unknown) return FunctionCache == Cache::Yes;
  ...
  void print(OutputStream &S) const {
    printLeft(S);
    if (RHSComponentCache != Cache::No) printRight(S);
  }
...
public:
  ...
  NodeArray(Node **Elements_, size_t NumElements_) : Elements(Elements_), NumElements(NumElements_) {}
...
public:
  DotSuffix(const Node *Prefix_, StringView Suffix_) : Node(KDotSuffix), Prefix(Prefix_), Suffix(Suffix_) {}
...
public:
  VendorExtQualType(const Node *Ty_, StringView Ext_) : Node(KVendorExtQualType), Ty(Ty_), Ext(Ext_) {}
...
protected:
  ...
  void printQuals(OutputStream &S) const {
    if (Quals & QualConst) S += " const";
    if (Quals & QualVolatile) S += " volatile";
    if (Quals & QualRestrict) S += " restrict";
  }

public:
  QualType(const Node *Child_, Qualifiers Quals_)
      : Node(KQualType, Child_->RHSComponentCache, Child_->ArrayCache, Child_->FunctionCache), Quals(Quals_),
        Child(Child_) {}

  template <typename Fn> void match(Fn F) const { F(Child, Quals); }

  bool hasRHSComponentSlow(OutputStream &S) const override { return Child->hasRHSComponent(S); }
  bool hasArraySlow(OutputStream &S) const override { return Child->hasArray(S); }
  bool hasFunctionSlow(OutputStream &S) const override { return Child->hasFunction(S); }
...
public:
  PostfixQualifiedType(Node *Ty_, StringView Postfix_) : Node(KPostfixQualifiedType), Ty(Ty_), Postfix(Postfix_) {}
...
public:
  ElaboratedTypeSpefType(StringView Kind_, Node *Child_) : Node(KElaboratedTypeSpefType), Kind(Kind_), Child(Child_) {}
...
struct AbiTagAttr : Node {
  ...
  AbiTagAttr(Node *Base_, StringView Tag_)
      : Node(KAbiTagAttr, Base_->RHSComponentCache, Base_->ArrayCache, Base_->FunctionCache), Base(Base_), Tag(Tag_) {}
...
public:
  ObjCProtoName(const Node *Ty_, StringView Protocol_) : Node(KObjCProtoName), Ty(Ty_), Protocol(Protocol_) {}
...
public:
  ...
  bool hasRHSComponentSlow(OutputStream &S) const override { return Pointee->hasRHSComponent(S); }

  void printLeft(OutputStream &s) const override {
    // We rewrite objc_object<SomeProtocol>* into id<SomeProtocol>.
    if (Pointee->getKind() != KObjCProtoName || !static_cast<const ObjCProtoName *>(Pointee)->isObjCObject()) {
      Pointee->printLeft(s);
      if (Pointee->hasArray(s)) s += " ";
      if (Pointee->hasArray(s) || Pointee->hasFunction(s)) s += "(";
      s += "*";
    } else {
...
class ReferenceType : public Node {
  ...
  std::pair<ReferenceKind, const Node *> collapse(OutputStream &S) const {
    ...
    for (;;) {
      ...
      if (SN->getKind() != KReferenceType) break;
...
public:
  ReferenceType(const Node *Pointee_, ReferenceKind RK_)
      : Node(KReferenceType, Pointee_->RHSComponentCache), Pointee(Pointee_), RK(RK_) {}
  ...
  void printLeft(OutputStream &s) const override {
    ...
    if (Collapsed.second->hasArray(s)) s += " ";
    if (Collapsed.second->hasArray(s) || Collapsed.second->hasFunction(s)) s += "(";
...
public:
  PointerToMemberType(const Node *ClassType_, const Node *MemberType_)
      : Node(KPointerToMemberType, MemberType_->RHSComponentCache), ClassType(ClassType_), MemberType(MemberType_) {}

  template <typename Fn> void match(Fn F) const { F(ClassType, MemberType); }

  bool hasRHSComponentSlow(OutputStream &S) const override { return MemberType->hasRHSComponent(S); }
  ...
  void printRight(OutputStream &s) const override {
    if (MemberType->hasArray(s) || MemberType->hasFunction(s)) s += ")";
...
public:
  ...
  void printRight(OutputStream &S) const override {
    if (S.back() != ']') S += " ";
    S += "[";
    if (Dimension) Dimension->print(S);
    S += "]";
    Base->printRight(S);
  }
};
...
public:
  FunctionType(
      const Node *Ret_, NodeArray Params_, Qualifiers CVQuals_, FunctionRefQual RefQual_, const Node *ExceptionSpec_)
      : Node(
            KFunctionType,
            /*RHSComponentCache=*/Cache::Yes, /*ArrayCache=*/Cache::No,
            /*FunctionCache=*/Cache::Yes),
        Ret(Ret_), Params(Params_), CVQuals(CVQuals_), RefQual(RefQual_), ExceptionSpec(ExceptionSpec_) {}

  template <typename Fn> void match(Fn F) const { F(Ret, Params, CVQuals, RefQual, ExceptionSpec); }
  ...
  void printRight(OutputStream &S) const override {
    ...
    if (CVQuals & QualConst) S += " const";
    if (CVQuals & QualVolatile) S += " volatile";
    if (CVQuals & QualRestrict) S += " restrict";
...
public:
  FunctionEncoding(
      const Node *Ret_, const Node *Name_, NodeArray Params_, const Node *Attrs_, Qualifiers CVQuals_,
      FunctionRefQual RefQual_)
      : Node(
            KFunctionEncoding,
            /*RHSComponentCache=*/Cache::Yes, /*ArrayCache=*/Cache::No,
            /*FunctionCache=*/Cache::Yes),
        Ret(Ret_), Name(Name_), Params(Params_), Attrs(Attrs_), CVQuals(CVQuals_), RefQual(RefQual_) {}

  template <typename Fn> void match(Fn F) const { F(Ret, Name, Params, Attrs, CVQuals, RefQual); }
  ...
  void printRight(OutputStream &S) const override {
    ...
    if (Attrs != nullptr) Attrs->print(S);
  }
};
...
enum class SpecialNameType {
  virtual_table,
  virtual_table_table,
  type_info,
  type_info_name,
  covariant_return_thunk,
  construction_virtual_table,
  thread_local_wrapper,
  thread_local_initialization,
  virtual_thunk,
  non_virtual_thunk,
  guard_variable,
  reference_temporary,
  invocation_function_block,
};

class SpecialName final : public Node {
  const SpecialNameType SpecialType;
  const StringView Special;
  const Node *Child;

public:
  SpecialName(SpecialNameType SpecialType, StringView Special_, const Node *Child_)
      : Node(KSpecialName), SpecialType(SpecialType), Special(Special_), Child(Child_) {}

  template <typename Fn> void match(Fn F) const { F(SpecialType, Special, Child); }
...
public:
  CtorVtableSpecialName(const Node *FirstType_, const Node *SecondType_)
      : Node(KCtorVtableSpecialName), FirstType(FirstType_), SecondType(SecondType_) {}
...
public:
  VectorType(const Node *BaseType_, Node *Dimension_) : Node(KVectorType), BaseType(BaseType_), Dimension(Dimension_) {}
...
public:
  ...
  void printLeft(OutputStream &S) const override {
    switch (Kind) {
    case TemplateParamKind::Type: S += "$T"; break;
    case TemplateParamKind::NonType: S += "$N"; break;
    case TemplateParamKind::Template: S += "$TT"; break;
    }
...
public:
  TemplateTemplateParamDecl(Node *Name_, NodeArray Params_)
      : Node(KTemplateTemplateParamDecl, Cache::Yes), Name(Name_), Params(Params_) {}
...
public:
  ParameterPack(NodeArray Data_) : Node(KParameterPack), Data(Data_) {
    ...
    if (std::all_of(Data.begin(), Data.end(), [](Node *P) { return P->ArrayCache == Cache::No; }))
      ArrayCache = Cache::No;
    if (std::all_of(Data.begin(), Data.end(), [](Node *P) { return P->FunctionCache == Cache::No; }))
      FunctionCache = Cache::No;
    if (std::all_of(Data.begin(), Data.end(), [](Node *P) { return P->RHSComponentCache == Cache::No; }))
  ...
  void printLeft(OutputStream &S) const override {
    initializePackExpansion(S);
    size_t Idx = S.CurrentPackIndex;
    if (Idx < Data.size()) Data[Idx]->printLeft(S);
  }
  void printRight(OutputStream &S) const override {
    initializePackExpansion(S);
    size_t Idx = S.CurrentPackIndex;
    if (Idx < Data.size()) Data[Idx]->printRight(S);
  }
};
...
struct ForwardTemplateReference : Node {
  ...
  ForwardTemplateReference(size_t Index_)
      : Node(KForwardTemplateReference, Cache::Unknown, Cache::Unknown, Cache::Unknown), Index(Index_) {}
...
public:
  ...
  StringView getBaseName() const override {
    switch (SSK) {
    case SpecialSubKind::allocator: return StringView("allocator");
    case SpecialSubKind::basic_string: return StringView("basic_string");
    case SpecialSubKind::string: return StringView("basic_string");
    case SpecialSubKind::istream: return StringView("basic_istream");
    case SpecialSubKind::ostream: return StringView("basic_ostream");
    case SpecialSubKind::iostream: return StringView("basic_iostream");
    }
  ...
  void printLeft(OutputStream &S) const override {
    switch (SSK) {
    case SpecialSubKind::allocator: S += "std::allocator"; break;
    case SpecialSubKind::basic_string: S += "std::basic_string"; break;
    case SpecialSubKind::string: S += "std::string"; break;
    case SpecialSubKind::istream: S += "std::istream"; break;
    case SpecialSubKind::ostream: S += "std::ostream"; break;
    case SpecialSubKind::iostream: S += "std::iostream"; break;
    }
  }
};
...
public:
  ...
  StringView getBaseName() const override {
    ...
    case SpecialSubKind::allocator: return StringView("allocator");
    case SpecialSubKind::basic_string: return StringView("basic_string");
    case SpecialSubKind::string: return StringView("string");
    case SpecialSubKind::istream: return StringView("istream");
    case SpecialSubKind::ostream: return StringView("ostream");
    case SpecialSubKind::iostream: return StringView("iostream");
    }
    DEMANGLE_UNREACHABLE;
  }
...
public:
  ClosureTypeName(NodeArray TemplateParams_, NodeArray Params_, StringView Count_)
      : Node(KClosureTypeName), TemplateParams(TemplateParams_), Params(Params_), Count(Count_) {}

  template <typename Fn> void match(Fn F) const { F(TemplateParams, Params, Count); }
...
public:
  PostfixExpr(const Node *Child_, StringView Operator_) : Node(KPostfixExpr), Child(Child_), Operator(Operator_) {}
...
public:
  EnclosingExpr(StringView Prefix_, Node *Infix_, StringView Postfix_)
      : Node(KEnclosingExpr), Prefix(Prefix_), Infix(Infix_), Postfix(Postfix_) {}
...
public:
  NewExpr(NodeArray ExprList_, Node *Type_, NodeArray InitList_, bool IsGlobal_, bool IsArray_)
      : Node(KNewExpr), ExprList(ExprList_), Type(Type_), InitList(InitList_), IsGlobal(IsGlobal_), IsArray(IsArray_) {}

  template <typename Fn> void match(Fn F) const { F(ExprList, Type, InitList, IsGlobal, IsArray); }

  void printLeft(OutputStream &S) const override {
    if (IsGlobal) S += "::operator ";
    S += "new";
    if (IsArray) S += "[]";
    S += ' ';
    if (!ExprList.empty()) {
...
public:
  ...
  void printLeft(OutputStream &S) const override {
    ...
    if (Init->getKind() != KBracedExpr && Init->getKind() != KBracedRangeExpr) S += " = ";
    Init->print(S);
  }
};
...
public:
  FoldExpr(bool IsLeftFold_, StringView OperatorName_, const Node *Pack_, const Node *Init_)
      : Node(KFoldExpr), Pack(Pack_), Init(Init_), OperatorName(OperatorName_), IsLeftFold(IsLeftFold_) {}

  template <typename Fn> void match(Fn F) const { F(IsLeftFold, OperatorName, Pack, Init); }
...
public:
  ...
  void printLeft(OutputStream &S) const override { S += Value ? StringView("true") : StringView("false"); }
};
...
public:
  ...
  void printLeft(OutputStream &S) const override {
    S += "[]";
    if (Type->getKind() == KClosureTypeName) static_cast<const ClosureTypeName *>(Type)->printDeclarator(S);
    S += "{...}";
  }
};
...
public:
  EnumLiteral(const Node *Ty_, StringView Integer_) : Node(KEnumLiteral), Ty(Ty_), Integer(Integer_) {}
...
public:
  IntegerLiteral(StringView Type_, StringView Value_) : Node(KIntegerLiteral), Type(Type_), Value(Value_) {}
...
constexpr Node::Kind getFloatLiteralKind(long double *) { return Node::KLongDoubleLiteral; }
} // namespace float_literal_impl
...
public:
  FloatLiteralImpl(StringView Contents_) : Node(KindForClass), Contents(Contents_) {}
...
template <class T, size_t N> class PODSmallVector {
  static_assert(std::is_trivially_copyable_v<T>, "T is required to be a plain old data type");
  ...
  void reserve(size_t NewCap) {
    ...
    if (isInline()) {
      ...
      if (Tmp == nullptr) std::terminate();
      std::copy(First, Last, Tmp);
      First = Tmp;
    } else {
      First = static_cast<T *>(std::realloc(First, NewCap * sizeof(T)));
      if (First == nullptr) std::terminate();
    }
    Last = First + S;
    Cap  = First + NewCap;
  }
...
template <typename Derived, typename Alloc> struct AbstractManglingParser {
  ...
  public:
    ScopedTemplateParamList(AbstractManglingParser *Parser)
        : Parser(Parser), OldNumTemplateParamLists(Parser->TemplateParams.size()) {
  ...
  AbstractManglingParser(const char *First_, const char *Last_) : First(First_), Last(Last_) {}
  ...
  void reset(const char *First_, const char *Last_) {
    ...
    for (int I = 0; I != 3; ++I) NumSyntheticTemplateParameters[I] = 0;
    ASTAllocator.reset();
  }
  ...
  char look(unsigned Lookahead = 0) {
    if (static_cast<size_t>(Last - First) <= Lookahead) return '\0';
  ...
  struct NameState {
    ...
    NameState(AbstractManglingParser *Enclosing) : ForwardTemplateRefsBegin(Enclosing->ForwardTemplateRefs.size()) {}
  };
  ...
  bool resolveForwardTemplateRefs(NameState &State) {
    ...
    for (; I < E; ++I) {
      ...
      if (TemplateParams.empty() || !TemplateParams[0] || Idx >= TemplateParams[0]->size()) return true;
...
//                          ::= <substitution>
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseName(NameState *State) {
  consumeIf('L'); // extension

  if (look() == 'N') return getDerived().parseNestedName(State);
  if (look() == 'Z') return getDerived().parseLocalName(State);

  //        ::= <unscoped-template-name> <template-args>
  if (look() == 'S' && look(1) != 't') {
    Node *S = getDerived().parseSubstitution();
    if (S == nullptr) return nullptr;
    if (look() != 'I') return nullptr;
...
Node *AbstractManglingParser<Derived, Alloc>::parseLocalName(NameState *State) {
  if (!consumeIf('Z')) return nullptr;
  Node *Encoding = getDerived().parseEncoding();
  if (Encoding == nullptr || !consumeIf('E')) return nullptr;

  if (consumeIf('s')) {
    First               = parse_discriminator(First, Last);
    auto *StringLitName = make<NameType>("string literal");
    if (!StringLitName) return nullptr;
    return make<LocalName>(Encoding, StringLitName);
  }

  if (consumeIf('d')) {
    parseNumber(true);
    if (!consumeIf('_')) return nullptr;
...
Node *AbstractManglingParser<Derived, Alloc>::parseUnqualifiedName(NameState *State) {
  ...
  if (Result != nullptr) Result = getDerived().parseAbiTags(Result);
  return Result;
}
...
Node *AbstractManglingParser<Derived, Alloc>::parseUnnamedTypeName(NameState *State) {
  ...
  if (State != nullptr) TemplateParams.clear();
  ...
  if (consumeIf("Ul")) {
    SwapAndRestore<size_t> SwapParams(ParsingLambdaParamsAtLevel, TemplateParams.size());
    ...
    if (TempParams.empty()) TemplateParams.pop_back();
...
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseSourceName(NameState *) {
  size_t Length = 0;
  if (parsePositiveInteger(&Length)) return nullptr;
  if (numLeft() < Length || Length == 0) return nullptr;
  StringView Name(First, First + Length);
  First += Length;
  if (Name.startsWith("_GLOBAL__N")) return make<NameType>("(anonymous namespace)");
...
Node *AbstractManglingParser<Derived, Alloc>::parseOperatorName(NameState *State) {
  ...
  case 'a':
    switch (look(1)) {
    case 'a': First += 2; return make<NameType>("operator&&");
    case 'd':
    case 'n': First += 2; return make<NameType>("operator&");
    case 'N': First += 2; return make<NameType>("operator&=");
    case 'S': First += 2; return make<NameType>("operator=");
    }
    return nullptr;
  case 'c':
    switch (look(1)) {
    case 'l': First += 2; return make<NameType>("operator()");
    case 'm': First += 2; return make<NameType>("operator,");
    case 'o': First += 2; return make<NameType>("operator~");
    //                   ::= cv <type>    # (cast)
    case 'v': {
      First += 2;
      ...
      SwapAndRestore<bool> SavePermit(
          PermitForwardTemplateReferences, PermitForwardTemplateReferences || State != nullptr);
  ...
  case 'd':
    switch (look(1)) {
    case 'a': First += 2; return make<NameType>("operator delete[]");
    case 'e': First += 2; return make<NameType>("operator*");
    case 'l': First += 2; return make<NameType>("operator delete");
    case 'v': First += 2; return make<NameType>("operator/");
    case 'V': First += 2; return make<NameType>("operator/=");
    }
    return nullptr;
  case 'e':
    ...
    case 'o': First += 2; return make<NameType>("operator^");
    case 'O': First += 2; return make<NameType>("operator^=");
    case 'q': First += 2; return make<NameType>("operator==");
    }
    return nullptr;
  case 'g':
    switch (look(1)) {
    case 'e': First += 2; return make<NameType>("operator>=");
    case 't': First += 2; return make<NameType>("operator>");
    }
    return nullptr;
  case 'i':
    if (look(1) == 'x') {
  ...
  case 'm':
    switch (look(1)) {
    case 'i': First += 2; return make<NameType>("operator-");
    case 'I': First += 2; return make<NameType>("operator-=");
    case 'l': First += 2; return make<NameType>("operator*");
    case 'L': First += 2; return make<NameType>("operator*=");
    case 'm': First += 2; return make<NameType>("operator--");
  ...
  case 'n':
    switch (look(1)) {
    case 'a': First += 2; return make<NameType>("operator new[]");
    case 'e': First += 2; return make<NameType>("operator!=");
    case 'g': First += 2; return make<NameType>("operator-");
    case 't': First += 2; return make<NameType>("operator!");
    case 'w': First += 2; return make<NameType>("operator new");
    }
    return nullptr;
  case 'o':
    switch (look(1)) {
    case 'o': First += 2; return make<NameType>("operator||");
    case 'r': First += 2; return make<NameType>("operator|");
    case 'R': First += 2; return make<NameType>("operator|=");
    }
    return nullptr;
  case 'p':
    ...
    case 'l': First += 2; return make<NameType>("operator+");
    case 'L': First += 2; return make<NameType>("operator+=");
    case 'p': First += 2; return make<NameType>("operator++");
    case 's': First += 2; return make<NameType>("operator+");
    case 't': First += 2; return make<NameType>("operator->");
    }
    return nullptr;
...
template <typename Derived, typename Alloc>
Node *AbstractManglingParser<Derived, Alloc>::parseCtorDtorName(Node *&SoFar, NameState *State) {
  ...
  if (consumeIf('C')) {
    ...
    if (look() != '1' && look() != '2' && look() != '3' && look() != '4' && look() != '5') return nullptr;
    int Variant = look() - '0';
    ++First;
    if (State) State->CtorDtorConversion = true;
    if (IsInherited) {
      if (getDerived().parseName(State) == nullptr) return nullptr;
    }
...
Node *AbstractManglingParser<Derived, Alloc>::parseNestedName(NameState *State) {
  ...
  while (!consumeIf('E')) {
    ...
    // <data-member-prefix> := <member source-name> [<template-args>] M
    if (consumeIf('M')) {
      if (SoFar == nullptr) return nullptr;
      continue;
    }

    //          ::= <template-param>
    if (look() == 'T') {
      if (!PushComponent(getDerived().parseTemplateParam())) return nullptr;
    ...
    if (look() == 'I') {
      Node *TA = getDerived().parseTemplateArgs(State != nullptr);
      if (TA == nullptr || SoFar == nullptr) return nullptr;
    ...
    if (look() == 'D' && (look(1) == 't' || look(1) == 'T')) {
      if (!PushComponent(getDerived().parseDecltype())) return nullptr;
      Subs.push_back(SoFar);
      continue;
    }
    ...
    if (look() == 'S' && look(1) != 't') {
      Node *S = getDerived().parseSubstitution();
      if (!PushComponent(S)) return nullptr;
      if (SoFar != S) Subs.push_back(S);
      continue;
    }
    ...
    if (look() == 'C' || (look() == 'D' && look(1) != 'C')) {
      if (SoFar == nullptr) return nullptr;
      if (!PushComponent(getDerived().parseCtorDtorName(SoFar, State))) return nullptr;
    ...
    if (!PushComponent(getDerived().parseUnqualifiedName(State))) return nullptr;
    Subs.push_back(SoFar);
  }

  if (SoFar == nullptr || Subs.empty()) return nullptr;
...
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseSimpleId() {
...
//                   ::= <simple-id>        # e.g., ~A<2*N>
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseDestructorName() {
  Node *Result;
  if (std::isdigit(look()))
...
//                   ::= <decltype>
//                   ::= <substitution>
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseUnresolvedType() {
  if (look() == 'T') {
...
//                                                                         # e.g. ~X or ~X<N-1>
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseBaseUnresolvedName() {
  if (std::isdigit(look())) return getDerived().parseSimpleId();

  if (consumeIf("dn")) return getDerived().parseDestructorName();

  consumeIf("on");

  Node *Oper = getDerived().parseOperatorName(/*NameState=*/nullptr);
  if (Oper == nullptr) return nullptr;
  if (look() == 'I') {
    Node *TA = getDerived().parseTemplateArgs();
    if (TA == nullptr) return nullptr;
...
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseUnresolvedName() {
  Node *SoFar = nullptr;
  ...
  if (consumeIf("srN")) {
    ...
    while (!consumeIf('E')) {
      ...
      if (Qual == nullptr) return nullptr;
  ...
  if (!consumeIf("sr")) {
    ...
    if (SoFar == nullptr) return nullptr;
    if (Global) SoFar = make<GlobalQualifiedName>(SoFar);
    return SoFar;
  }
...
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseAbiTags(Node *N) {
  while (consumeIf('B')) {
...
StringView AbstractManglingParser<Alloc, Derived>::parseNumber(bool AllowNegative) {
  const char *Tmp = First;
  if (AllowNegative) consumeIf('n');
  if (numLeft() == 0 || !std::isdigit(*First)) return StringView();
  while (numLeft() != 0 && std::isdigit(*First)) ++First;
...
template <typename Alloc, typename Derived> StringView AbstractManglingParser<Alloc, Derived>::parseBareSourceName() {
  size_t Int = 0;
  if (parsePositiveInteger(&Int) || numLeft() < Int) return StringView();
...
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseFunctionType() {
  Qualifiers CVQuals = parseCVQualifiers();

  Node *ExceptionSpec = nullptr;
  if (consumeIf("Do")) {
    ExceptionSpec = make<NameType>("noexcept");
    if (!ExceptionSpec) return nullptr;
  } else if (consumeIf("DO")) {
    Node *E = getDerived().parseExpr();
    if (E == nullptr || !consumeIf('E')) return nullptr;
    ...
    ExceptionSpec = make<DynamicExceptionSpec>(popTrailingNodeArray(SpecsBegin));
    if (!ExceptionSpec) return nullptr;
  }

  consumeIf("Dx"); // transaction safe

  if (!consumeIf('F')) return nullptr;
  ...
  size_t ParamsBegin                 = Names.size();
  while (true) {
    if (consumeIf('E')) break;
    if (consumeIf('v')) continue;
    if (consumeIf("RE")) {
  ...
  NodeArray Params = popTrailingNodeArray(ParamsBegin);
  return make<FunctionType>(ReturnType, Params, CVQuals, ReferenceQualifier, ExceptionSpec);
}
...
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseVectorType() {
  if (!consumeIf("Dv")) return nullptr;
  if (look() >= '1' && look() <= '9') {
    Node *DimensionNumber = make<NameType>(parseNumber());
    if (!DimensionNumber) return nullptr;
    if (!consumeIf('_')) return nullptr;
    if (consumeIf('p')) return make<PixelVectorType>(DimensionNumber);
...
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseDecltype() {
  if (!consumeIf('D')) return nullptr;
  if (!consumeIf('t') && !consumeIf('T')) return nullptr;
  Node *E = getDerived().parseExpr();
  if (E == nullptr) return nullptr;
  if (!consumeIf('E')) return nullptr;
  return make<EnclosingExpr>("decltype(", E, ")");
}

// <array-type> ::= A <positive dimension number> _ <element type>
//              ::= A [<dimension expression>] _ <element type>
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseArrayType() {
  if (!consumeIf('A')) return nullptr;
  ...
  if (std::isdigit(look())) {
    ...
    Node *DimExpr = getDerived().parseExpr();
    if (DimExpr == nullptr) return nullptr;
    if (!consumeIf('_')) return nullptr;
    Dimension = DimExpr;
  }
...
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parsePointerToMemberType() {
  if (!consumeIf('M')) return nullptr;
...
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseClassEnumType() {
  StringView ElabSpef;
  if (consumeIf("Ts"))
    ElabSpef = "struct";
  ...
  if (!ElabSpef.empty()) return make<ElaboratedTypeSpefType>(ElabSpef, Name);
...
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseQualifiedType() {
  if (consumeIf('U')) {
    ...
    if (Qual.startsWith("objcproto")) {
      ...
      StringView Proto;
      {
        SwapAndRestore<const char *> SaveFirst(First, ProtoSourceName.begin()), SaveLast(Last, ProtoSourceName.end());
  ...
  Node *Ty         = getDerived().parseType();
  if (Ty == nullptr) return nullptr;
  if (Quals != QualNone) Ty = make<QualType>(Ty, Quals);
  return Ty;
}
...
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseType() {
  Node *Result = nullptr;
  ...
  case 'K': {
    ...
    if (look(AfterQuals) == 'F' ||
        (look(AfterQuals) == 'D' && (look(AfterQuals + 1) == 'o' || look(AfterQuals + 1) == 'O' ||
  ...
  // <builtin-type> ::= v    # void
  case 'v': ++First; return make<NameType>("void");
  //                ::= w    # wchar_t
  case 'w': ++First; return make<NameType>("wchar_t");
  //                ::= b    # bool
  case 'b': ++First; return make<NameType>("bool");
  //                ::= c    # char
  case 'c': ++First; return make<NameType>("char");
  //                ::= a    # signed char
  case 'a': ++First; return make<NameType>("signed char");
  //                ::= h    # unsigned char
  case 'h': ++First; return make<NameType>("unsigned char");
  //                ::= s    # short
  case 's': ++First; return make<NameType>("short");
  //                ::= t    # unsigned short
  case 't': ++First; return make<NameType>("unsigned short");
  //                ::= i    # int
  case 'i': ++First; return make<NameType>("int");
  //                ::= j    # unsigned int
  case 'j': ++First; return make<NameType>("unsigned int");
  //                ::= l    # long
  case 'l': ++First; return make<NameType>("long");
  //                ::= m    # unsigned long
  case 'm': ++First; return make<NameType>("unsigned long");
  //                ::= x    # long long, __int64
  case 'x': ++First; return make<NameType>("long long");
  //                ::= y    # unsigned long long, __int64
  case 'y': ++First; return make<NameType>("unsigned long long");
  //                ::= n    # __int128
  case 'n': ++First; return make<NameType>("__int128");
  //                ::= o    # unsigned __int128
  case 'o': ++First; return make<NameType>("unsigned __int128");
  //                ::= f    # float
  case 'f': ++First; return make<NameType>("float");
  //                ::= d    # double
  case 'd': ++First; return make<NameType>("double");
  //                ::= e    # long double, __float80
  case 'e': ++First; return make<NameType>("long double");
  //                ::= g    # __float128
  case 'g': ++First; return make<NameType>("__float128");
  //                ::= z    # ellipsis
  case 'z': ++First; return make<NameType>("...");
  ...
  case 'D':
    ...
    case 'd': First += 2; return make<NameType>("decimal64");
    //                ::= De   # IEEE 754r decimal floating point (128 bits)
    case 'e': First += 2; return make<NameType>("decimal128");
    //                ::= Df   # IEEE 754r decimal floating point (32 bits)
    case 'f': First += 2; return make<NameType>("decimal32");
    //                ::= Dh   # IEEE 754r half-precision floating point (16 bits)
    case 'h': First += 2; return make<NameType>("decimal16");
    //                ::= Di   # char32_t
    case 'i': First += 2; return make<NameType>("char32_t");
    //                ::= Ds   # char16_t
    case 's': First += 2; return make<NameType>("char16_t");
    //                ::= Du   # char8_t (C++2a, not yet in the Itanium spec)
    case 'u': First += 2; return make<NameType>("char8_t");
    //                ::= Da   # auto (in dependent new-expressions)
    case 'a': First += 2; return make<NameType>("auto");
    //                ::= Dc   # decltype(auto)
    case 'c': First += 2; return make<NameType>("decltype(auto)");
    //                ::= Dn   # std::nullptr_t (i.e., decltype(nullptr))
    case 'n': First += 2; return make<NameType>("std::nullptr_t");
    ...
    case 'x': Result = getDerived().parseFunctionType(); break;
    }
    break;
  //             ::= <function-type>
  case 'F': {
  ...
  if (Result != nullptr) Subs.push_back(Result);
  return Result;
}
...
Node *AbstractManglingParser<Derived, Alloc>::parseIntegerLiteral(StringView Lit) {
  ...
  if (!Tmp.empty() && consumeIf('E')) return make<IntegerLiteral>(Lit, Tmp);
  return nullptr;
}

// <CV-Qualifiers> ::= [r] [V] [K]
template <typename Alloc, typename Derived> Qualifiers AbstractManglingParser<Alloc, Derived>::parseCVQualifiers() {
  Qualifiers CVR = QualNone;
  if (consumeIf('r')) CVR |= QualRestrict;
  if (consumeIf('V')) CVR |= QualVolatile;
  if (consumeIf('K')) CVR |= QualConst;
  return CVR;
}

// <function-param> ::= fp <top-level CV-Qualifiers> _                                     # L == 0, first parameter
//                  ::= fp <top-level CV-Qualifiers> <parameter-2 non-negative number> _   # L == 0, second and later
//                  parameters
...
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseFunctionParam() {
  if (consumeIf("fpT")) return make<NameType>("this");
  if (consumeIf("fp")) {
    parseCVQualifiers();
  ...
  if (consumeIf("fL")) {
    if (parseNumber().empty()) return nullptr;
    if (!consumeIf('p')) return nullptr;
    parseCVQualifiers();
    StringView Num = parseNumber();
    if (!consumeIf('_')) return nullptr;
    return make<FunctionParam>(Num);
...
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseNewExpr() {
  bool Global  = consumeIf("gs");
  bool IsArray = look(1) == 'a';
  if (!consumeIf("nw") && !consumeIf("na")) return nullptr;
...
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseConversionExpr() {
  if (!consumeIf("cv")) return nullptr;
  Node *Ty;
  {
...
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseExprPrimary() {
  if (!consumeIf('L')) return nullptr;
  switch (look()) {
  case 'w': ++First; return getDerived().parseIntegerLiteral("wchar_t");
  case 'b':
    if (consumeIf("b0E")) return make<BoolExpr>(0);
    if (consumeIf("b1E")) return make<BoolExpr>(1);
    return nullptr;
  case 'c': ++First; return getDerived().parseIntegerLiteral("char");
  case 'a': ++First; return getDerived().parseIntegerLiteral("signed char");
  case 'h': ++First; return getDerived().parseIntegerLiteral("unsigned char");
  case 's': ++First; return getDerived().parseIntegerLiteral("short");
  case 't': ++First; return getDerived().parseIntegerLiteral("unsigned short");
  case 'i': ++First; return getDerived().parseIntegerLiteral("");
  case 'j': ++First; return getDerived().parseIntegerLiteral("u");
  case 'l': ++First; return getDerived().parseIntegerLiteral("l");
  case 'm': ++First; return getDerived().parseIntegerLiteral("ul");
  case 'x': ++First; return getDerived().parseIntegerLiteral("ll");
  case 'y': ++First; return getDerived().parseIntegerLiteral("ull");
  case 'n': ++First; return getDerived().parseIntegerLiteral("__int128");
  case 'o': ++First; return getDerived().parseIntegerLiteral("unsigned __int128");
  case 'f': ++First; return getDerived().template parseFloatingLiteral<float>();
  case 'd': ++First; return getDerived().template parseFloatingLiteral<double>();
  case 'e': ++First;
  ...
  case '_':
    if (consumeIf("_Z")) {
      ...
      if (R != nullptr && consumeIf('E')) return R;
    }
    return nullptr;
  case 'A': {
    Node *T = getDerived().parseType();
    ...
    if (consumeIf('E')) return make<StringLiteral>(T);
    return nullptr;
  }
  case 'D':
    if (consumeIf("DnE")) return make<NameType>("nullptr");
    return nullptr;
  case 'T':
  ...
  case 'U': {
    ...
    if (look(1) != 'l') return nullptr;
    Node *T = parseUnnamedTypeName(nullptr);
    if (!T || !consumeIf('E')) return nullptr;
...
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseBracedExpr() {
  if (look() == 'd') {
    switch (look(1)) {
    case 'i': {
      First += 2;
    ...
    case 'X': {
      ...
      if (RangeBegin == nullptr) return nullptr;
      Node *RangeEnd = getDerived().parseExpr();
      if (RangeEnd == nullptr) return nullptr;
      Node *Init = getDerived().parseBracedExpr();
      if (Init == nullptr) return nullptr;
...
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseFoldExpr() {
  if (!consumeIf('f')) return nullptr;
  ...
  if (IsLeftFold && Init) std::swap(Pack, Init);
...
//              ::= sP <template-arg>* E                                 # sizeof...(T), size of a captured template
//              parameter pack from an alias template
...
//              ::= fr <binary-operator-name> <expression>
//              ::= <expr-primary>
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseExpr() {
  bool Global = consumeIf("gs");
  if (numLeft() < 2) return nullptr;

  switch (*First) {
  case 'L': return getDerived().parseExprPrimary();
  case 'T': return getDerived().parseTemplateParam();
  case 'f': {
    // Disambiguate a fold expression from a <function-param>.
    if (look(1) == 'p' || (look(1) == 'L' && std::isdigit(look(2)))) return getDerived().parseFunctionParam();
    return getDerived().parseFoldExpr();
  }
  case 'a':
    switch (First[1]) {
    case 'a': First += 2; return getDerived().parseBinaryExpr("&&");
  ...
  case 'd':
    ...
    case 'e': First += 2; return getDerived().parsePrefixExpr("*");
    case 'l': {
      First += 2;
      Node *E = getDerived().parseExpr();
      if (E == nullptr) return E;
    ...
    case 'n': return getDerived().parseUnresolvedName();
    case 's': {
      First += 2;
      Node *LHS = getDerived().parseExpr();
      if (LHS == nullptr) return nullptr;
    ...
    case 'v': First += 2; return getDerived().parseBinaryExpr("/");
    case 'V': First += 2; return getDerived().parseBinaryExpr("/=");
    }
    return nullptr;
  case 'e':
  ...
  case 'g':
    switch (First[1]) {
    case 'e': First += 2; return getDerived().parseBinaryExpr(">=");
  ...
  case 'm':
    switch (First[1]) {
    case 'i': First += 2; return getDerived().parseBinaryExpr("-");
    case 'I': First += 2; return getDerived().parseBinaryExpr("-=");
    case 'l': First += 2; return getDerived().parseBinaryExpr("*");
    case 'L': First += 2; return getDerived().parseBinaryExpr("*=");
    case 'm':
      First += 2;
      if (consumeIf('_')) return getDerived().parsePrefixExpr("--");
      Node *Ex = getDerived().parseExpr();
      if (Ex == nullptr) return nullptr;
  ...
  case 'n':
    switch (First[1]) {
    case 'a':
    case 'w': return getDerived().parseNewExpr();
    case 'e': First += 2; return getDerived().parseBinaryExpr("!=");
  ...
  case 'p':
    ...
    case 's': First += 2; return getDerived().parsePrefixExpr("+");
    case 't': {
      First += 2;
  ...
  case 's':
    ...
    case 'r': return getDerived().parseUnresolvedName();
    case 't': {
      First += 2;
      Node *Ty = getDerived().parseType();
      if (Ty == nullptr) return Ty;
  ...
  case 't':
    ...
    case 'r': First += 2; return make<NameType>("throw");
    case 'w': {
      First += 2;
      Node *Ex = getDerived().parseExpr();
      if (Ex == nullptr) return nullptr;
  ...
  case '7':
  case '8':
  case '9': return getDerived().parseUnresolvedName();
  }
...
template <typename Alloc, typename Derived> bool AbstractManglingParser<Alloc, Derived>::parseCallOffset() {
  // Just scan through the call offset, we never add this information into the
  // output.
  if (consumeIf('h')) return parseNumber(true).empty() || !consumeIf('_');
  if (consumeIf('v'))
...
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseSpecialName() {
  switch (look()) {
  case 'T':
    switch (look(1)) {
    ...
    case 'V': {
      ...
      Node *Ty = getDerived().parseType();
      if (Ty == nullptr) return nullptr;
      return make<SpecialName>(SpecialNameType::virtual_table, "vtable for ", Ty);
    }
    // TT <type>    # VTT structure (construction vtable index)
    case 'T': {
      First += 2;
      Node *Ty = getDerived().parseType();
      if (Ty == nullptr) return nullptr;
      return make<SpecialName>(SpecialNameType::virtual_table_table, "VTT for ", Ty);
    }
    // TI <type>    # typeinfo structure
    case 'I': {
      First += 2;
      Node *Ty = getDerived().parseType();
      if (Ty == nullptr) return nullptr;
      return make<SpecialName>(SpecialNameType::type_info, "typeinfo for ", Ty);
    }
    // TS <type>    # typeinfo name (null-terminated byte string)
    case 'S': {
      First += 2;
      Node *Ty = getDerived().parseType();
      if (Ty == nullptr) return nullptr;
      return make<SpecialName>(SpecialNameType::type_info_name, "typeinfo name for ", Ty);
    }
    // Tc <call-offset> <call-offset> <base encoding>
    case 'c': {
      First += 2;
      if (parseCallOffset() || parseCallOffset()) return nullptr;
      Node *Encoding = getDerived().parseEncoding();
      if (Encoding == nullptr) return nullptr;
      return make<SpecialName>(SpecialNameType::covariant_return_thunk, "covariant return thunk to ", Encoding);
    }
    ...
    case 'C': {
      ...
      if (FirstType == nullptr) return nullptr;
      if (parseNumber(true).empty() || !consumeIf('_')) return nullptr;
      Node *SecondType = getDerived().parseType();
      if (SecondType == nullptr) return nullptr;
    ...
    case 'W': {
      ...
      Node *Name = getDerived().parseName();
      if (Name == nullptr) return nullptr;
      return make<SpecialName>(SpecialNameType::thread_local_wrapper, "thread-local wrapper routine for ", Name);
    }
    // TH <object name> # Thread-local initialization
    case 'H': {
      First += 2;
      ...
      return make<SpecialName>(
          SpecialNameType::thread_local_initialization, "thread-local initialization routine for ", Name);
    }
    // T <call-offset> <base encoding>
    default: {
      ++First;
      bool IsVirt = look() == 'v';
      if (parseCallOffset()) return nullptr;
      Node *BaseEncoding = getDerived().parseEncoding();
      if (BaseEncoding == nullptr) return nullptr;
      if (IsVirt)
        return make<SpecialName>(SpecialNameType::virtual_thunk, "virtual thunk to ", BaseEncoding);
      else
        return make<SpecialName>(SpecialNameType::non_virtual_thunk, "non-virtual thunk to ", BaseEncoding);
    }
    }
  case 'G':
    switch (look(1)) {
    ...
    case 'V': {
      ...
      Node *Name = getDerived().parseName();
      if (Name == nullptr) return nullptr;
      return make<SpecialName>(SpecialNameType::guard_variable, "guard variable for ", Name);
    }
    ...
    case 'R': {
      ...
      if (!consumeIf('_') && ParsedSeqId) return nullptr;
      return make<SpecialName>(SpecialNameType::reference_temporary, "reference temporary for ", Name);
    }
    }
  }
  return nullptr;
...
//            ::= <data name>
//            ::= <special-name>
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseEncoding() {
  if (look() == 'G' || look() == 'T') return getDerived().parseSpecialName();
  ...
  if (resolveForwardTemplateRefs(NameInfo)) return nullptr;

  if (IsEndOfEncoding()) return Name;
  ...
  if (consumeIf('v'))
    return make<FunctionEncoding>(
        ReturnType, Name, NodeArray(), Attrs, NameInfo.CVQualifiers, NameInfo.ReferenceQualifier);
  ...
  return make<FunctionEncoding>(
      ReturnType, Name, popTrailingNodeArray(ParamsBegin), Attrs, NameInfo.CVQualifiers, NameInfo.ReferenceQualifier);
}
...
#if defined(__mips__) && defined(__mips_n64) || defined(__aarch64__) || defined(__wasm__)
...
// <seq-id> ::= <0-9A-Z>+
template <typename Alloc, typename Derived> bool AbstractManglingParser<Alloc, Derived>::parseSeqId(size_t *Out) {
  if (!(look() >= '0' && look() <= '9') && !(look() >= 'A' && look() <= 'Z')) return true;
...
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseSubstitution() {
  if (!consumeIf('S')) return nullptr;
  ...
  //                ::= S <seq-id> _
  size_t Index = 0;
  if (parseSeqId(&Index)) return nullptr;
  ++Index;
  if (!consumeIf('_') || Index >= Subs.size()) return nullptr;
  return Subs[Index];
}
...
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseTemplateParam() {
  if (!consumeIf('T')) return nullptr;

  size_t Level = 0;
  if (consumeIf('L')) {
    if (parsePositiveInteger(&Level)) return nullptr;
    ++Level;
    if (!consumeIf('_')) return nullptr;
  }
  ...
  size_t Index = 0;
  if (!consumeIf('_')) {
    if (parsePositiveInteger(&Index)) return nullptr;
    ++Index;
    if (!consumeIf('_')) return nullptr;
  }
  ...
  if (Level >= TemplateParams.size() || !TemplateParams[Level] || Index >= TemplateParams[Level]->size()) {
    ...
    if (ParsingLambdaParamsAtLevel == Level && Level <= TemplateParams.size()) {
      ...
      if (Level == TemplateParams.size()) TemplateParams.push_back(nullptr);
...
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseTemplateParamDecl() {
...
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parseTemplateArg() {
  switch (look()) {
  case 'X': {
    ++First;
    Node *Arg = getDerived().parseExpr();
    if (Arg == nullptr || !consumeIf('E')) return nullptr;
    return Arg;
  }
  case 'J': {
    ++First;
  ...
  default: return getDerived().parseType();
  }
}
...
template <typename Derived, typename Alloc>
Node *AbstractManglingParser<Derived, Alloc>::parseTemplateArgs(bool TagTemplates) {
  if (!consumeIf('I')) return nullptr;
  ...
  while (!consumeIf('E')) {
    if (TagTemplates) {
      ...
      if (Arg->getKind() == Node::KTemplateArgumentPack) {
        TableEntry = make<ParameterPack>(static_cast<TemplateArgumentPack *>(TableEntry)->getElements());
        if (!TableEntry) return nullptr;
      }
...
template <typename Derived, typename Alloc> Node *AbstractManglingParser<Derived, Alloc>::parse() {
  if (consumeIf("_Z") || consumeIf("__Z")) {
    ...
    if (numLeft() != 0) return nullptr;
    return Encoding;
  }

  if (consumeIf("___Z") || consumeIf("____Z")) {
    Node *Encoding = getDerived().parseEncoding();
    if (Encoding == nullptr || !consumeIf("_block_invoke")) return nullptr;
    bool RequireNumber = consumeIf('_');
    if (parseNumber().empty() && RequireNumber) return nullptr;
    if (look() == '.') First = Last;
    if (numLeft() != 0) return nullptr;
    return make<SpecialName>(SpecialNameType::invocation_function_block, "invocation function for block in ", Encoding);
  }
...
template <typename Alloc> struct ManglingParser : AbstractManglingParser<ManglingParser<Alloc>, Alloc> {
  using AbstractManglingParser<ManglingParser<Alloc>, Alloc>::AbstractManglingParser;
};

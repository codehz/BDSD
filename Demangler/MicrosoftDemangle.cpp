...
static bool startsWithDigit(StringView S) { return !S.empty() && std::isdigit(S.front()); }
...
static bool startsWithLocalScopePattern(StringView S) {
  if (!S.consumeFront('?')) return false;

  size_t End = S.find('?');
  if (End == StringView::npos) return false;
  StringView Candidate = S.substr(0, End);
  if (Candidate.empty()) return false;

  // \?[0-9]\?
  // ?@? is the discriminator 0.
  if (Candidate.size() == 1) return Candidate[0] == '@' || (Candidate[0] >= '0' && Candidate[0] <= '9');

  // If it's not 0-9, then it's an encoded number terminated with an @
  if (Candidate.back() != '@') return false;
  ...
  if (Candidate[0] < 'B' || Candidate[0] > 'P') return false;
  Candidate = Candidate.dropFront();
  while (!Candidate.empty()) {
    if (Candidate[0] < 'A' || Candidate[0] > 'P') return false;
    Candidate = Candidate.dropFront();
  }
...
static std::pair<Qualifiers, PointerAffinity> demanglePointerCVQualifiers(StringView &MangledName) {
  if (MangledName.consumeFront("$$Q")) return std::make_pair(Q_None, PointerAffinity::RValueReference);

  switch (MangledName.popFront()) {
  case 'A': return std::make_pair(Q_None, PointerAffinity::Reference);
  case 'P': return std::make_pair(Q_None, PointerAffinity::Pointer);
  case 'Q': return std::make_pair(Q_Const, PointerAffinity::Pointer);
  case 'R': return std::make_pair(Q_Volatile, PointerAffinity::Pointer);
  case 'S': return std::make_pair(Qualifiers(Q_Const | Q_Volatile), PointerAffinity::Pointer);
  }
...
SpecialTableSymbolNode *Demangler::demangleSpecialTableSymbolNode(StringView &MangledName, SpecialIntrinsicKind K) {
  ...
  if (!MangledName.consumeFront('@')) STSN->TargetName = demangleFullyQualifiedTypeName(MangledName);
  return STSN;
}
...
LocalStaticGuardVariableNode *Demangler::demangleLocalStaticGuard(StringView &MangledName, bool IsThread) {
  ...
  if (!MangledName.empty()) LSGI->ScopeIndex = demangleUnsigned(MangledName);
  return LSGVN;
}
...
VariableSymbolNode *
Demangler::demangleUntypedVariable(ArenaAllocator &Arena, StringView &MangledName, StringView VariableName) {
  NamedIdentifierNode *NI = synthesizeNamedIdentifier(Arena, VariableName);
  QualifiedNameNode *QN   = demangleNameScopeChain(MangledName, NI);
  VariableSymbolNode *VSN = Arena.alloc<VariableSymbolNode>();
  VSN->Name               = QN;
  if (MangledName.consumeFront("8")) return VSN;
...
FunctionSymbolNode *Demangler::demangleInitFiniStub(StringView &MangledName, bool IsDestructor) {
  ...
  if (MangledName.consumeFront('?')) IsKnownStaticDataMember = true;
  ...
  if (Symbol->kind() == NodeKind::VariableSymbol) {
    ...
    for (int I = 0; I < AtCount; ++I) {
      if (MangledName.consumeFront('@')) continue;
      Error = true;
      return nullptr;
    }

    FSN = demangleFunctionEncoding(MangledName);
    if (FSN) FSN->Name = synthesizeQualifiedName(Arena, DSIN);
  } else {
...
SymbolNode *Demangler::demangleSpecialIntrinsic(StringView &MangledName) {
  ...
  case SpecialIntrinsicKind::LocalStaticGuard: return demangleLocalStaticGuard(MangledName, /*IsThread=*/false);
  case SpecialIntrinsicKind::LocalStaticThreadGuard: return demangleLocalStaticGuard(MangledName, /*IsThread=*/true);
  case SpecialIntrinsicKind::RttiTypeDescriptor: {
    TypeNode *T = demangleType(MangledName, QualifierMangleMode::Result);
    if (Error) break;
    if (!MangledName.consumeFront("@8")) break;
    if (!MangledName.empty()) break;
  ...
  case SpecialIntrinsicKind::DynamicInitializer: return demangleInitFiniStub(MangledName, /*IsDestructor=*/false);
  case SpecialIntrinsicKind::DynamicAtexitDestructor: return demangleInitFiniStub(MangledName, /*IsDestructor=*/true);
  ...
  case SpecialIntrinsicKind::UdtReturning:
    ...
    break;
  case SpecialIntrinsicKind::Unknown: DEMANGLE_UNREACHABLE; // Never returned by consumeSpecialIntrinsicKind.
  }
  Error = true;
  return nullptr;
}
...
IdentifierNode *Demangler::demangleFunctionIdentifierCode(StringView &MangledName, FunctionIdentifierCodeGroup Group) {
  ...
  case FunctionIdentifierCodeGroup::Under:
    return Arena.alloc<IntrinsicFunctionIdentifierNode>(translateIntrinsicFunctionCode(MangledName.popFront(), Group));
...
SymbolNode *Demangler::demangleEncodedSymbol(StringView &MangledName, QualifiedNameNode *Name) {
  ...
  if (UQN->kind() == NodeKind::ConversionOperatorIdentifier) {
    ...
    if (FSN) COIN->TargetType = FSN->Signature->ReturnType;
  }
  return FSN;
}
...
SymbolNode *Demangler::parse(StringView &MangledName) {
  ...
  if (MangledName.startsWith('.')) return demangleTypeinfoName(MangledName);

  if (MangledName.startsWith("??@")) return demangleMD5Name(MangledName);
  ...
  if (SymbolNode *SI = demangleSpecialIntrinsic(MangledName)) return SI;

  return demangleDeclarator(MangledName);
}

TagTypeNode *Demangler::parseTagUniqueName(StringView &MangledName) {
  if (!MangledName.consumeFront(".?A")) return nullptr;
...
int64_t Demangler::demangleSigned(StringView &MangledName) {
  ...
  uint64_t Number              = 0;
  std::tie(Number, IsNegative) = demangleNumber(MangledName);
  if (Number > INT64_MAX) Error = true;
...
void Demangler::memorizeString(StringView S) {
  if (Backrefs.NamesCount >= BackrefContext::Max) return;
  for (size_t i = 0; i < Backrefs.NamesCount; ++i)
    if (auto named = dynamic_cast<NamedIdentifierNode const *>(Backrefs.Names[i]))
      if (named->Name == S) return;
  NamedIdentifierNode *N                = Arena.alloc<NamedIdentifierNode>();
  N->Name                               = S;
  Backrefs.Names[Backrefs.NamesCount++] = N;
}

IdentifierNode *Demangler::demangleBackRefName(StringView &MangledName) {
...
void Demangler::memorizeIdentifier(IdentifierNode *Identifier) {
  if (Backrefs.NamesCount >= BackrefContext::Max) return;
  for (size_t i = 0; i < Backrefs.NamesCount; ++i)
    if (Identifier->operator==(*Backrefs.Names[i])) return;
  Backrefs.Names[Backrefs.NamesCount++] = Identifier;
  // for (size_t i = 0; i < Backrefs.NamesCount; ++i)
  //  if (Identifier == Backrefs.Names[i]->Name) return;
...
IdentifierNode *Demangler::demangleTemplateInstantiationName(StringView &MangledName, NameBackrefBehavior NBB) {
  ...
  if (!Error) Identifier->TemplateParams = demangleTemplateParameterList(MangledName);
...
uint8_t Demangler::demangleCharLiteral(StringView &MangledName) {
  ...
  if (!MangledName.startsWith('?')) return MangledName.popFront();

  MangledName = MangledName.dropFront();
  if (MangledName.empty()) goto CharLiteralError;

  if (MangledName.consumeFront('$')) {
    // Two hex digits
    if (MangledName.size() < 2) goto CharLiteralError;
    StringView Nibbles = MangledName.substr(0, 2);
    if (!isRebasedHexDigit(Nibbles[0]) || !isRebasedHexDigit(Nibbles[1])) goto CharLiteralError;
  ...
  if (MangledName[0] >= 'A' && MangledName[0] <= 'Z') {
    char Lookup[26] = {'\xC1', '\xC2', '\xC3', '\xC4', '\xC5', '\xC6', '\xC7', '\xC8', '\xC9',
                       '\xCA', '\xCB', '\xCC', '\xCD', '\xCE', '\xCF', '\xD0', '\xD1', '\xD2',
...
wchar_t Demangler::demangleWcharLiteral(StringView &MangledName) {
  ...
  if (Error || MangledName.empty()) goto WCharLiteralError;
  C2 = demangleCharLiteral(MangledName);
  if (Error) goto WCharLiteralError;
...
static unsigned guessCharByteSize(const uint8_t *StringBytes, unsigned NumChars, uint64_t NumBytes) {
  ...
  if (NumBytes < 32) {
    ...
    if (TrailingNulls >= 4 && NumBytes % 4 == 0) return 4;
    if (TrailingNulls >= 2) return 2;
    return 1;
  }
  ...
  if (Nulls >= 2 * NumChars / 3 && NumBytes % 4 == 0) return 4;
  if (Nulls >= NumChars / 3) return 2;
  return 1;
}
...
FunctionSymbolNode *Demangler::demangleVcallThunkNode(StringView &MangledName) {
  ...
  if (!Error) Error = !MangledName.consumeFront("$B");
  if (!Error) VTIN->OffsetInVTable = demangleUnsigned(MangledName);
  if (!Error) Error = !MangledName.consumeFront('A');
  if (!Error) FSN->Signature->CallConvention = demangleCallingConvention(MangledName);
...
EncodedStringLiteralNode *Demangler::demangleStringLiteral(StringView &MangledName) {
  ...
  if (!MangledName.consumeFront("@_")) goto StringLiteralError;
  if (MangledName.empty()) goto StringLiteralError;
  ...
  if (Error || IsNegative || StringByteSize < (IsWcharT ? 2 : 1)) goto StringLiteralError;

  // CRC 32 (always 8 characters plus a terminator)
  CrcEndPos = MangledName.find('@');
  if (CrcEndPos == StringView::npos) goto StringLiteralError;
  ...
  if (IsWcharT) {
    ...
    if (StringByteSize > 64) Result->IsTruncated = true;

    while (!MangledName.consumeFront('@')) {
      if (MangledName.size() < 2) goto StringLiteralError;
      wchar_t W = demangleWcharLiteral(MangledName);
      if (StringByteSize != 2 || Result->IsTruncated) outputEscapedChar(OS, W);
      StringByteSize -= 2;
      if (Error) goto StringLiteralError;
    }
  } else {
    ...
    while (!MangledName.consumeFront('@')) {
      if (MangledName.size() < 1 || BytesDecoded >= MaxStringByteLength) goto StringLiteralError;
    ...
    for (unsigned CharIndex = 0; CharIndex < NumChars; ++CharIndex) {
      ...
      if (CharIndex + 1 < NumChars || Result->IsTruncated) outputEscapedChar(OS, NextChar);
    }
  }
...
StringView Demangler::demangleSimpleString(StringView &MangledName, bool Memorize) {
  StringView S;
  for (size_t i = 0; i < MangledName.size(); ++i) {
    if (MangledName[i] != '@') continue;
    if (i == 0) break;
    S           = MangledName.substr(0, i);
    MangledName = MangledName.dropFront(i + 1);

    if (Memorize) memorizeString(S);
    return S;
  }
...
NamedIdentifierNode *Demangler::demangleLocallyScopedNamePiece(StringView &MangledName) {
  ...
  LocalNamedIdentifierNode *Identifier = Arena.alloc<LocalNamedIdentifierNode>();
  MangledName.consumeFront('?');
  uint64_t Number              = 0;
  bool IsNegative              = false;
  std::tie(Number, IsNegative) = demangleNumber(MangledName);
...
IdentifierNode *Demangler::demangleUnqualifiedTypeName(StringView &MangledName, bool Memorize) {
  ...
  if (MangledName.startsWith("?$")) return demangleTemplateInstantiationName(MangledName, NBB_Template);
...
IdentifierNode *Demangler::demangleUnqualifiedSymbolName(StringView &MangledName, NameBackrefBehavior NBB) {
  if (startsWithDigit(MangledName)) return demangleBackRefName(MangledName);
  if (MangledName.startsWith("?$")) return demangleTemplateInstantiationName(MangledName, NBB);
  if (MangledName.startsWith('?')) return demangleFunctionIdentifierCode(MangledName);
...
IdentifierNode *Demangler::demangleNameScopePiece(StringView &MangledName) {
  ...
  if (MangledName.startsWith("?A")) return demangleAnonymousNamespaceName(MangledName);
...
FuncClass Demangler::demangleFunctionClass(StringView &MangledName) {
  ...
  case '$': {
    ...
    if (MangledName.consumeFront('R')) VFlag = FuncClass(VFlag | FC_VirtualThisAdjustEx);
    if (MangledName.empty()) break;
    switch (MangledName.popFront()) {
...
std::pair<Qualifiers, bool> Demangler::demangleQualifiers(StringView &MangledName) {
  ...
  switch (MangledName.popFront()) {
  // Member qualifiers
  case 'Q': return std::make_pair(Q_None, true);
  case 'R': return std::make_pair(Q_Const, true);
  case 'S': return std::make_pair(Q_Volatile, true);
  case 'T': return std::make_pair(Qualifiers(Q_Const | Q_Volatile), true);
  // Non-Member qualifiers
  case 'A': return std::make_pair(Q_None, false);
  case 'B': return std::make_pair(Q_Const, false);
  case 'C': return std::make_pair(Q_Volatile, false);
  case 'D': return std::make_pair(Qualifiers(Q_Const | Q_Volatile), false);
  }
  Error = true;
  return std::make_pair(Q_None, false);
}
...
TypeNode *Demangler::demangleType(StringView &MangledName, QualifierMangleMode QMM) {
  ...
  if (QMM == QualifierMangleMode::Mangle) {
    ...
    if (MangledName.consumeFront('?')) std::tie(Quals, IsMember) = demangleQualifiers(MangledName);
  }
...
bool Demangler::demangleThrowSpecification(StringView &MangledName) {
  if (MangledName.consumeFront("_E")) return true;
  if (MangledName.consumeFront('Z')) return false;
...
FunctionSignatureNode *Demangler::demangleFunctionType(StringView &MangledName, bool HasThisQuals) {
  ...
  if (!IsStructor) FTy->ReturnType = demangleType(MangledName, QualifierMangleMode::Result);
...
FunctionSymbolNode *Demangler::demangleFunctionEncoding(StringView &MangledName) {
  ...
  if (MangledName.consumeFront("$$J0")) ExtraFlags = FC_ExternC;
...
CustomTypeNode *Demangler::demangleCustomType(StringView &MangledName) {
  ...
  if (!MangledName.consumeFront('@')) Error = true;
  if (Error) return nullptr;
  return CTN;
}
...
PrimitiveTypeNode *Demangler::demanglePrimitiveType(StringView &MangledName) {
  if (MangledName.consumeFront("$$T")) return Arena.alloc<PrimitiveTypeNode>(PrimitiveKind::Nullptr);
...
Qualifiers Demangler::demanglePointerExtQualifiers(StringView &MangledName) {
  Qualifiers Quals = Q_None;
  if (MangledName.consumeFront('E')) Quals = Qualifiers(Quals | Q_Pointer64);
  if (MangledName.consumeFront('I')) Quals = Qualifiers(Quals | Q_Restrict);
  if (MangledName.consumeFront('F')) Quals = Qualifiers(Quals | Q_Unaligned);
...
NodeArrayNode *Demangler::demangleFunctionParameterList(StringView &MangledName, bool &IsVariadic) {
  ...
  if (MangledName.consumeFront('X')) return nullptr;
  ...
  if (MangledName.consumeFront('@')) return NA;
    ...
    *Current = Arena.alloc<NodeList>();
    ...
    if (MangledName.consumeFront("$$Y")) {
      ...
      TP.N = demangleType(MangledName, QualifierMangleMode::Mangle);
        ...
        MangledName.startsWith("$1") || MangledName.startsWith("$H") || MangledName.startsWith("$I") ||
...
void Demangler::dumpBackReferences() {
  ...
  OutputStream OS;
  if (!initializeOutputStream(nullptr, nullptr, OS, 1024)) std::terminate();
  ...
  for (size_t I = 0; I < Backrefs.NamesCount; ++I) {
    OS.setCurrentPosition(0);
    Backrefs.Names[I]->output(OS, OF_Default);
    std::printf("  [%d] - %.*s\n", (int) I, (int) OS.getCurrentPosition(), OS.getBuffer());
  }
  std::free(OS.getBuffer());
...
char *llvm::microsoftDemangle(
  ...
  if (!D.Error && NMangled) *NMangled = Name.begin() - MangledName;
